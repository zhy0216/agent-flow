...
export async function setupDependencies(
  ...
  async function fingerprint(source: string) {
    ...
    for (const path of untracked)
      files.push({
        path,
        sha256: hash(await readFile(await checkedPath(source, path))),
      });
  ...
  async function assertReceipt(
    ...
    const receiptFile = Bun.file(
      join(source, ".git", "agent-flow-source.json"),
    );
  ...
  async function prepareSource(dependency: Dependency) {
    ...
    for (const patch of dependency.patches) {
      const path = await checkedPath(root, patch.file);
    ...
    const sources = join(dependencyRoot, "sources");
    await mkdir(sources, { recursive: true });
    await checkedPath(dependencyRoot, "sources");
    const source = join(
      dependencyRoot,
      "sources",
    ...
    if (
      ...
      await checkedPath(sources, relative(sources, source));
    ...
    try {
      ...
      await Bun.write(
        ...
        JSON.stringify({
          ...identity,
          fingerprint: await fingerprint(staging),
        }),
      );
  ...
  for (const dependency of lock.dependencies) {
    ...
    await assertReceipt(source, identity);
    for (const pkg of dependency.packages) {
      const directory = await checkedPath(source, pkg.directory);
      await validatePackage({ ...pkg, directory });
...
if (import.meta.main) await setupDependencies();
