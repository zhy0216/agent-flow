...
| [06-worker-query-indexes.md](06-worker-query-indexes.md) | P2 | hard | codex（继承默认） | gpt-6-astra / max | 待确认事件部分索引、增量迁移和查询计划证据。 |
| [07-dependency-audit.md](07-dependency-audit.md) | P2 | medium | codex（继承默认） | gpt-6-astra / xhigh | 处理 esbuild 告警并建立可重复审计 gate。 |
...
6. `06-worker-query-indexes.md` — 依赖：无。
7. `07-dependency-audit.md` — 依赖：无。
